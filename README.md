...
### StampTicker
Тот же `Stamp`, но сохраняет счёт времени на базе `millis()` после синхронизации. Имеет тикер, нужно вызывать его в `loop()`. Сигналит раз в секунду в 0 миллисекунд новой секунды. При задержках в программе будет сигналить чаще, чтобы догнать отставание, чтобы не пропускать секунды и все из обработать. Если при синхронизации время отстаёт от реального - тикер будет вызван подряд несколько раз на весь период рассинхронизации. если спешит - не будет вызываться, пока реальное время не догонит внутреннее.
